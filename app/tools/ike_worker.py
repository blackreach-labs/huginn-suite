...
class IKEWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            self.signals.output.emit(f"<p style='color: #00BFFF;'>[IKE SCAN] Starting {h(self.scan_type)} on {h(self.target)}:{h(self.port)}</p><br>")
            ...
            if basic_results.get('error'):
                self.signals.output.emit(f"<p style='color: #FF6B6B;'>[ERROR] {h(basic_results['error'])}</p><br>")
                ...
                if results['ike_accessible']:
                    self.signals.output.emit(f"<p style='color: #00FF41;'>IKE service accessible on port {h(self.port)}</p><br>")
                else:
                    self.signals.output.emit(f"<p style='color: #FFAA00;'>IKE service may not be accessible on port {h(self.port)}</p><br>")
            ...
            if results.get('ike_scan_available') and self.scan_type in ["Detailed Info", "Transform Enum"]:
                ...
                if self.scan_type == "Transform Enum":
                    ...
                    if transform_results.get('error'):
                        self.signals.output.emit(f"<p style='color: #FF6B6B;'>[ERROR] Transform scan: {h(transform_results['error'])}</p><br>")
                    else:
                        ...
                        if results['transforms']:
                            ...
                            for transform in results['transforms']:
                                self.signals.output.emit(f"<p style='color: #87CEEB;'>  - {h(transform)}</p><br>")
                        else:
                    ...
                    if detailed_results.get('error'):
                        self.signals.output.emit(f"<p style='color: #FF6B6B;'>[ERROR] Detailed scan: {h(detailed_results['error'])}</p><br>")
                    else:
                        results.update({
                        ...
                        if results['handshake_type']:
                            self.signals.output.emit(f"<p style='color: #00FF41;'>Handshake Type: {h(results['handshake_type'])}</p><br>")
                        ...
                        if results['vendor_ids']:
                            ...
                            for vid in results['vendor_ids']:
                                self.signals.output.emit(f"<p style='color: #87CEEB;'>  - {h(vid)}</p><br>")
