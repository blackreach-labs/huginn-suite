# app/tools/enhanced_huggin_scanner.py
"""
Enhanced Huggin Scanner with Advanced Analysis Integration
...
from ..core.huggin_results_analyzer import huggin_analyzer
...
class EnhancedHugginScanner(HugginVulnScanner):
    """Enhanced Huggin scanner with advanced analysis capabilities"""
    ...
    def _store_enhanced_results(self, scan_results):
        ...
        try:
            scan_id = f"enhanced_huggin_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            
            # Store original scan results
            centralized_scan_data.add_scan_result(
                scan_id=scan_id,
                tenant_id=self.tenant_id,
                scan_type="huggin_enhanced",
                target=self.target_url,
                scanner="enhanced_huggin_scanner",
                result_data={
                    'scan_results': scan_results,
                    'analysis_results': self.analysis_results,
                    'attack_chains': [
    ...
    def _display_enhanced_results(self):
        ...
        print("\n" + "=" * 60)
        print("ENHANCED HUGGIN SCANNER RESULTS")
        print("=" * 60)
        ...
        print("Use the Huggin Dashboard for detailed interactive analysis.")
    
    def generate_comprehensive_report(self, format='html'):
        """Generate comprehensive report in specified format"""
        if format == 'html':
            return huggin_analyzer.generate_detailed_report(self.analysis_results)
        elif format == 'json':
            return json.dumps({
                'analysis_results': self.analysis_results,
                'attack_chains': [
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python enhanced_huggin_scanner.py <target_url> [profile] [tenant_id]")
        print("Example: python enhanced_huggin_scanner.py https://example.com normal my_company")
        return
    ...
    scanner = EnhancedHugginScanner(target_url, profile, tenant_id=tenant_id)
    ...
    with open(f'huggin_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.html', 'w') as f:
        f.write(html_report)
    
    # JSON export
    json_report = scanner.generate_comprehensive_report('json')
    with open(f'huggin_data_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json', 'w') as f:
        f.write(json_report)
    
    # Executive summary
    exec_report = scanner.generate_comprehensive_report('executive')
    with open(f'huggin_executive_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt', 'w') as f:
        f.write(exec_report)
