...
async def advanced_scan():
    ...
    scanner.config_manager.config['custom_headers'] = {
        ...
        'User-Agent': 'Huginn Security Scanner'
    }
...
# Specific profile
python -m app.tools.huginn_vuln_scanner --target https://example.com --profile aggressive

# With output file
python -m app.tools.huginn_vuln_scanner --target https://example.com --output results.json --format html
```
...
*The Huginn Advanced Security Scanner represents the evolution of automated security testing, combining traditional techniques with cutting-edge AI to provide unparalleled vulnerability discovery and assessment capabilities.*