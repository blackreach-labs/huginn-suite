# Huginn Scanner Quick Reference

## Command Line Usage

```bash
# Basic scan
python -m app.tools.huginn_vuln_scanner https://example.com

# With profile
python -m app.tools.huginn_vuln_scanner https://example.com aggressive

# With custom config
python -m app.tools.huginn_vuln_scanner https://example.com normal /path/to/config.yaml
```
...
async def scan():
    scanner = HuginnVulnScanner('https://example.com', 'normal')
...
pipeline {
    stages {
        stage('Security Scan') {
            steps {
                script {
                    sh 'python -m app.tools.huginn_vuln_scanner ${TARGET_URL} light'
                }
            }
        }
    }
...
- name: Security Scan
  run: |
    python -m app.tools.huginn_vuln_scanner ${{ env.TARGET_URL }} light
```
...
data = centralized_scan_data.get_scan_data('tenant', 'huginn_vuln')
```
...
python test_huginn_comprehensive.py https://dvwa.lab.local
```
...
This quick reference provides essential information for using the Huginn Advanced Security Scanner effectively.