# Huggin Advanced Security Scanner - Complete Documentation
...
custom_headers:             # Custom HTTP headers
  User-Agent: 'Huggin Scanner v2.0'
...
```python
from app.core.huggin_config_manager import ConfigManager
...
scanner = HugginVulnScanner('https://example.com', profile='normal', verify_ssl=False)
```
...
async def comprehensive_test():
    ...
    for profile in profiles:
        ...
        scanner = HugginVulnScanner(target, profile=profile, verify_ssl=False)
        results = await scanner.scan()
...
async def ci_cd_scan():
    scanner = HugginVulnScanner('https://staging.example.com', profile='light')
    results = await scanner.scan()
...
The scanner integrates with the Huggin asset management system:
